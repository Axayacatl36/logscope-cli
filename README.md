...
*   **Live Filtering**: `--live` buffers the last 2000 lines and lets you adjust search/regex/module/level filters interactively with the keyboard (`/` search, `r` regex search, `m` opens a multi-select list of every module seen so far — `↑`/`↓` move, `enter` toggle, `a` all/none, `m` confirm — `1`-`4` toggle DEBUG/INFO/WARN/ERROR visibility, `c` clear filters, `p` clear buffer, `q` quit) — filters are re-applied retroactively over the whole buffer, not just new lines. `--wrap-width` applies to the module picker's names too, wrapping long ones onto an indented continuation line. Requires a real terminal (a file/device path, not piped stdin).
