...
*   **Live Filtering**: `--live` buffers the last 2000 lines and lets you adjust search/regex/module/level filters interactively with the keyboard (`/` search, `r` toggle regex, `m` module, `1`-`9`/`0` toggle a level's visibility, `c` clear, `q` quit) — filters are re-applied retroactively over the whole buffer, not just new lines. Requires a real terminal (a file/device path, not piped stdin).
